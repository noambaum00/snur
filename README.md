...
התמונה הבאה מציגה דוגמה לחיבור אחד ממודולי המיקרופון על גבי breadboard:

<img src="https://github.com/user-attachments/assets/207622c2-d442-40c6-abda-3999b0bdaf15" alt="חיבור מיקרופון על breadboard" width="420">

### חיבור 4 מיקרופונים אנלוגיים ל־MCP3008

הפרויקט מניח 4 מיקרופונים אנלוגיים, למשל מודולי `MAX9814`, שמחוברים לכניסות האנלוגיות של `MCP3008`.

| מיקרופון | פין `OUT` | ערוץ ב־MCP3008 | ערוץ בקונפיגורציה |
| --- | --- | --- | --- |
| Mic 1 | אות אנלוגי | `CH0` | `0` |
| Mic 2 | אות אנלוגי | `CH1` | `1` |
| Mic 3 | אות אנלוגי | `CH2` | `2` |
| Mic 4 | אות אנלוגי | `CH3` | `3` |

לכל אחד מארבעת המיקרופונים:

- `Vdd` → ‏`3.3V` של ה־Raspberry Pi
- `GND` → ‏`GND` משותף
- `OUT` → ערוץ מתאים ב־`MCP3008` (`CH0`–`CH3`)
- `GAIN` / `AR` → אופציונלי; אפשר להשאיר לא מחובר אם לא נדרש כיוון ידני

### חיבור MCP3008 ל־Raspberry Pi
...
הגדרת ברירת המחדל בקוד היא:

- `channels = [0, 1, 2, 3]`
- `spi_bus = 0`
- `spi_device = 0`

ודא שכל הרכיבים עובדים על `3.3V` ושיש `GND` משותף לכל המערכת.
